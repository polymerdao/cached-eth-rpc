    ...
    fn extract_cache_key(&self, params: &Value) -> anyhow::Result<Option<String>> {
        ...
        let slot =
            match params[1] {
                ...
                Value::Number(ref n) => {
                    n.as_u64().ok_or(anyhow::anyhow!("params[1] not a valid slot value"))?.to_string()
                }
...
mod test {
    ...
    #[test]
    fn test_normal_case() {
        let params = json!(["0xC310e760778ECBca4C65B6C559874757A4c4Ece0", "0x1234", "0x1234"]);
        let cache_key = HANDLER.extract_cache_key(&params).unwrap().unwrap();
        assert_eq!(cache_key, "0x1234-0xc310e760778ecbca4c65b6c559874757a4c4ece0-4660");

        let params = json!(["0xC310e760778ECBca4C65B6C559874757A4c4Ece0", 1234, "0x1234"]);
        let cache_key = HANDLER.extract_cache_key(&params).unwrap().unwrap();
        assert_eq!(cache_key, "0x1234-0xc310e760778ecbca4c65b6c559874757a4c4ece0-1234");
    ...
    fn test_invalid_slot() {
        let params = json!(["0xC310e760778ECBca4C65B6C559874757A4c4Ece0", "0x1234gg", "0x1234"]);
        assert_eq!(
            HANDLER.extract_cache_key(&params).unwrap_err().to_string(),
